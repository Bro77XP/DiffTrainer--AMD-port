    ...
    def start(cls):
        ...
        if not hparams['infer']:  # train
            ...
            if ckpt_path is not None:
                backed_up = _backup_incompatible_ckpt_if_needed(task, ckpt_path)
                if backed_up:
                    ckpt_path = None
...
def _backup_incompatible_ckpt_if_needed(task, ckpt_path):
    """Guard against accidentally overwriting a checkpoint of a DIFFERENT model
    type. The current model has a ``category`` (e.g. 'acoustic' / 'variance')
    that is stored in every checkpoint. If the latest checkpoint in the work
    directory belongs to a different category, or its state_dict is otherwise
    incompatible, back up ALL ``.ckpt`` files in that directory to a sibling
    folder (collision-safe names, never overwriting an existing backup) and let
    training start from scratch.

    Returns True if a backup was performed (caller should train from scratch).
    """
    cur_category = getattr(task.model, 'category', None)
    try:
    ...
    incompatible = False
    reason = ''
    ckpt_category = ckpt.get('category')
    if cur_category is not None and ckpt_category is not None:
        if ckpt_category != cur_category:
            incompatible = True
            reason = f"category '{ckpt_category}' != current '{cur_category}'"
    else:
        # No category info: fall back to a state_dict key compatibility check.
        sd = ckpt.get('state_dict')
        if sd is not None:
            model_keys = set(task.model.state_dict().keys())
            ckpt_keys = set(sd.keys())
            missing = model_keys - ckpt_keys
            unexpected = ckpt_keys - model_keys
            if missing or unexpected:
                incompatible = True
                reason = f'{len(missing)} missing, {len(unexpected)} unexpected keys'
        else:
            incompatible = True
            reason = 'no state_dict/category found'

    if not incompatible:
        return False

    work_dir = pathlib.Path(ckpt_path).parent
    backup_dir = work_dir.parent / (work_dir.name + '_incompatible_ckpt_backup')
    backup_dir.mkdir(parents=True, exist_ok=True)

    # The work directory holds a different model type. Move ALL .ckpt files out
    # (collision-safe names, never overwriting an existing backup) so training
    # starts cleanly and the old model is preserved rather than destroyed.
    moved = []
    for ckpt_file in sorted(work_dir.glob('*.ckpt')):
        dst = backup_dir / ckpt_file.name
        if dst.exists():
            stem, suffix = ckpt_file.stem, ckpt_file.suffix
            i = 1
            while dst.exists():
                dst = backup_dir / f'{stem}.{i}{suffix}'
                i += 1
        shutil.move(str(ckpt_file), str(dst))
        moved.append(ckpt_file.name)

    print(
        f'| WARNING: checkpoint \'{ckpt_path}\' is incompatible with the current model '
        f'({reason}). '
        f'Moved {len(moved)} checkpoint(s) (NOT overwritten) to \'{backup_dir}\' '
        f'and training will start from scratch.'
    )
    return True
