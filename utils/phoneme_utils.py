...
class PhonemeDictionary:
    ...
    def encode_one(self, phone, lang=None):
        ...
        if lang is None or not self._multi_langs or phone in self._phone_to_id:
            ...
            key = phone
        if key not in self._phone_to_id:
            vf_key = f'{lang}/vf' if lang else 'vf'
            if vf_key not in self._phone_to_id:
                raise KeyError(
                    f"Phoneme '{phone}' not in dictionary and no vocal-fry fallback "
                    f"('{vf_key}') available."
                )
            if key not in _VF_WARNED:
                print(f"Warning: phoneme '{key}' not in dictionary, mapping to vocal fry ({vf_key})")
                _VF_WARNED.add(key)
            key = vf_key
